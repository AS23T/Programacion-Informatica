    ...
    public partial class frmPráctico4 : Form
    {
        ...
        private void btnCalcular_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int elMayor = ObtenerMayor(valores);
                int elMenor = ObtenerMenor(valores);
                double elPromedio = ObtenerPromedio(valores);

                txtMayor.Text = ObtenerMayor(valores).ToString();
                txtMenor.Text = elMenor.ToString();
                txtPromedio.Text = elPromedio.ToString();
            }
            catch (Exception)
            {
                MessageBox.Show("error en los datos. asegurese de usar espacios simples", "error");
            }
